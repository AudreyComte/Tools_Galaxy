...
# Charger le contenu du premier fichier dans un data.frame
dataframe_total_count <- read.table(file_list[[1]], header = FALSE, sep = "\t")

# Copier les deux premières colonnes dans le data.frame
if (ncol(dataframe_total_count) >= 2) {
  # Sélectionner les deux premières colonnes
  columns_1_2 <- dataframe_total_count[, 1:2]
} else {
  # Si le fichier a moins de deux colonnes, mettre à NA pour la troisième colonne
  columns_1_2 <- cbind(dataframe_total_count, NA)
}

# Vérifier si la liste contient plus d'un fichier
if (length(file_list) > 1) {
  # Charger le contenu du deuxième fichier
  second_file_content <- read.table(file_list[[2]], header = FALSE, sep = "\t")

  # Copier seulement la deuxième colonne du deuxième fichier dans la troisième colonne du data.frame
  if (ncol(second_file_content) >= 2) {
    # Sélectionner la deuxième colonne
    column_2_second_file <- second_file_content[, 2]
  } else {
    # Si le fichier a moins de deux colonnes, mettre à NA pour la troisième colonne
    column_2_second_file <- NA
  }

  # Ajouter la deuxième colonne du deuxième fichier dans la troisième colonne du data.frame
  dataframe_total_count <- cbind(dataframe_total_count, column_2_second_file)
}
